...
if(isset($_POST['submit'])){
	...
	$adminApprove = $_POST['adminApprove'];
	if($uStatus==0)
	{
		$uStatus = 1;
		$adminApprove = 1;
    ...
    mysqli_query($con,"UPDATE users SET fname='$fname',mname='$mname',lname='$lname',photo='$user_image_path',email='$email',contactNo='$contactNo',address='$address',city='$city',state='$state',zip='$zip',username='$username',password='$password',updatedDt='$curretDate',uStatus='$uStatus',adminApprove='$adminApprove' where id=$editUserId") or die(mysqli_error($con));
	...
	header('location: '.$redirectURL_user.'?page=profile'); exit;
}
?>